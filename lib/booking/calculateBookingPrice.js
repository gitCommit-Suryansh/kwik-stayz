...
export function calculateBookingPrice({
  ...
  // Use Math.floor to ensure integer values as per requirement (round down even for x.9)
  const extraGuestPricePerNight = Math.floor(
    (basePrice * extraGuestPricing) / 100
  );
  ...
  const breakdown = rooms.map((room, index) => {
    const roomBasePrice = Math.floor(basePrice * nights);
    const roomExtraCost = Math.floor(
      room.extras * extraGuestPricePerNight * nights
    );
